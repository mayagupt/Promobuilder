...
function App() {
  ...
  const handleUserMessage = (goalText = null) => {
    ...
    if (step === 0) {
      ...
      newMessages.push({ sender: 'bot', text: 'What is your budget for this promotion?' });
      setChat(prev => [...prev, ...newMessages]);
    } else if (step === 1) {
      setFollowUps(prev => ({ ...prev, budget: message.toLowerCase(), step: 2 }));
      newMessages.push({ sender: 'bot', text: 'Is there a specific margin you want to maintain? (optional)' });
      setChat(prev => [...prev, ...newMessages]);
    } else if (step === 2) {
      setFollowUps(prev => ({ ...prev, margin: message, step: 3 }));
      newMessages.push({ sender: 'bot', text: 'When do you want to run this promotion?' });
      setChat(prev => [...prev, ...newMessages]);
    } else if (step === 3) {
